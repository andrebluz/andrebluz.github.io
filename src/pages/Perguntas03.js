...
import WelcomeBanner from '../partials/dashboard/PerguntasTecnologia';
...
const Perguntas = () => {
  ...
  .then(function(snapshot) {
    ...
    snapshot.child("3/Perguntas").forEach((snap) => {
 ...
 // contador2 = Object.getOwnPropertyNames(snapshot.child("1/Perguntas").val()).sort()
  ...
  setLoading(true)
    ...
    useEffect(() => {
      ...
      firebase.database().ref('temas/3/Perguntas').on('value', snapshot => { 
...
var add2 = () => {
  firebase.database().ref("temas").once("value").then(function(snapshot) {
    ...
    contador2 = Object.getOwnPropertyNames(snapshot.child("3/Perguntas").val()).sort()
    console.log(Object.getOwnPropertyNames(snapshot.child("3/Perguntas").val()).sort().length)
...
var addOrEdit = (obj) => {
  if (currentId == '')
  firebase.database().ref('temas/3/Perguntas').push(
                obj,
                err => {
                    if (err)
                        console.log(err)
        ...
        firebase.database().ref(`temas/3/Perguntas/`+currentId).set(
                obj,
                err => {
                    if (err)
                        console.log(err)
      ...
      if (currentId == '')
{
  firebase.database().ref("temas").once("value").then(function(snapshot) {
    ...
    if (snapshot.child("3/Perguntas").val() != null) {

    contador2 = Object.getOwnPropertyNames(snapshot.child("3/Perguntas").val()).sort().length
    } else{
      contador2 = 0
    }
 
    firebase.database().ref('temas/3/Perguntas/0'+(contador2+1)).set(
              obj,
              err => {
                  if (err)
                      console.log(err)
  ...
  const onDelete = id => {
    if (window.confirm('Are you sure to delete this record?')) {
      firebase.database().ref('temas/3/Perguntas/'+id).remove(
            err => {
                if (err)
                    console.log(err)
                else
...
var onDelete2 = tempkey => {
    ...
    firebase.database().ref('temas/3/Perguntas/'+tempkey).remove(
          err => {
              if (err)
                  console.log(err)
              else
