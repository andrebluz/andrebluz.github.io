...
const Perguntas = () => {
  ...
  .then(function(snapshot) {
    ...
    snapshot.child("1/Perguntas").forEach((snap) => {
 ...
 // contador2 = Object.getOwnPropertyNames(snapshot.child("1/Perguntas").val()).sort()
  ...
  setLoading(true)
    ...
    useEffect(() => {
      ...
      firebase.database().ref('temas/1/Perguntas').on('value', snapshot => { 
...
var addOrEdit = (obj) => {
  if (currentId == '')
  firebase.database().ref('temas/1/Perguntas').push(
                obj,
                err => {
                    if (err)
                        console.log(err)
        ...
        firebase.database().ref(`temas/1/Perguntas/`+currentId).set(
                obj,
                err => {
                    if (err)
                        console.log(err)
      ...
      if (currentId == '')
{
  firebase.database().ref("temas").once("value").then(function(snapshot) {
    ...
    if (snapshot.child("1/Perguntas").val() != null) {

    contador2 = Object.getOwnPropertyNames(snapshot.child("1/Perguntas").val()).sort().length
    } else{
      contador2 = 0
    }
 
    firebase.database().ref('temas/1/Perguntas/0'+(contador2+1)).set(
              obj,
              err => {
                  if (err)
                      console.log(err)
  ...
  const onDelete = id => {
    if (window.confirm('Are you sure to delete this record?')) {
      firebase.database().ref('temas/1/Perguntas/'+id).remove(
            err => {
                if (err)
                    console.log(err)
                else
...
var onDelete2 = tempkey => {
    ...
    firebase.database().ref('temas/1/Perguntas/'+tempkey).remove(
          err => {
              if (err)
                  console.log(err)
              else
                    ...
                    <table className=" table-auto" >
                        <thead>
                            <tr className="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                                ...
                                <th className="py-3 px-6 text-left">Resposta:</th>
                        ...
                        <tbody className="text-gray-600 text-sm font-light">
                            {
                                Object.keys(contactObjects).map((key) => (
                                    <tr key={key} className="border-b border-gray-200 hover:bg-gray-200">
                                      <td><div className="text-sm font-semibold text-white px-1.5 bg-green-500 rounded-full pull-right numberclass">{key}</div></td>
                                        <td className="py-3 px-6 text-left " width="45%">{contactObjects[key].Pergunta}</td>
                                        <td className="py-3 px-6 text-left">{contactObjects[key].Resposta}</td>
