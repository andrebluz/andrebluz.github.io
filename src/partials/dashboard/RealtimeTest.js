...
function DashboardCard05() {
  ...
  const data = [
    1,7,3,4,1,5,4,10,10,10,10,6,6,6,9
  ];
  const now = new Date();
  ...
  const [slicedLabels, setSlicedLabels] = useState(generateDates2().slice(0, range).reverse());
  ...
  useEffect(() => {
    ...
    setSlicedLabels(([x, ...slicedLabels]) => [...slicedLabels,  new Date(datao + 2000)]);
